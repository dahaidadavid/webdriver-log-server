...
var IOSTools = require('appium-ios-log');
var IOSLog = IOSTools.IOSLog;
var IOSCrashLog = IOSTools.IOSCrashLog;
...
helpers.startLogCapture = async function (sim,udid,show) {
  if (!_.isEmpty(this.logs)) {
...
helpers.stopLogCapture = async function(){
	  try {
    await this.logs.syslog.stopCapture();
  } catch (err) {
    logger.warn("Try to stop log capture is error.");
    return;
  }
}
